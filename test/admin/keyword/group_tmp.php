...
function del(id) {
    ...
    {
        $.ajax({
            url:"<?=$this->SYSTEM_path("admin/keyword/group_del")?>",
                data:"group_id="+id,
                dataType:"html",
                type:"post",
...
function add() {
    window.location.href="<?=$this->SYSTEM_path("admin/keyword/group_add")?>";
}
</script>
</head>
<body>
...
<form name="form2" id="form2" method="post" action="" onSubmit="return check()">
  <table cellspacing="1" cellpadding="5" width="100%" style="margin:6px 0 0 0;">
   <tr>
  <td colspan="13" style="color:#FF0000;font-weight:bold;"><input type="button" value="添加关键词分组" onclick="add()">&nbsp;当前：维护关键词分组&nbsp;</td>
  </tr>
  <tr class="head">
  <td width="15%">ID</td>
  <td width="15%">名称</td>
  ...
  <tr sid="search_id" vid="<?=$item["id"]?>">
      ...
      <td><?=$item["keynums"]?></td>
      <td><a href="<?=$this->SYSTEM_path("admin/keyword/group_update/{$item["id"]}/?backurl={$tmp["backurl"]}")?>">编辑</a>&nbsp;<a href="<?=$this->SYSTEM_path("admin/keyword/group_change/{$item["id"]}/?backurl={$tmp["backurl"]}")?>">关键词转移</a>&nbsp;<input type="button" value="删除" onclick="del(<?=$item["id"]?>)"></td>
  </tr>
      <?php
  }
  }
  ?>
<tr>
<td colspan="13"><input type="button" value="添加关键词分组" onclick="add()">&nbsp;
