...
<form name="form2" id="form2" method="post" action="" onSubmit="return check()">
  ...
  <tr sid="search_id" vid="<?=$item["id"]?>">
      ...
      <td><?=$item["fans"]?></td>
      <td><?=$item["message"]?></td>
      <td><a href="<?=$this->SYSTEM_path("admin/store/fans/{$item["id"]}/0/?backurl={$tmp["backurl"]}")?>">查看粉丝</a>&nbsp;<a href="<?=$this->SYSTEM_path("admin/store/message/{$item["id"]}/0/?backurl={$tmp["backurl"]}")?>">查看推送记录</a></td>
  </tr>
      <?php
  }
  }
...
<tr>
<td colspan="13">
<?php echo "共{$tmp["info"]["nums"]}<span=\"#FF0000\">店铺</span>&nbsp;&nbsp;当前{$tmp["info"]["current_page"]}/{$tmp["info"]["pages"]}页"; ?>
